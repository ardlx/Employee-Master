               ...
               <v-container fluid>
                    <v-card>
                         ...
                         >
                              ...
                              <template v-slot:item="props">
                                   <tr :style="props.item.DeletedDate != null ? 'color: #b71c1c;' : ''">
                                        <td>{{props.item.ProductCategoryDesc}}</td>
                         ...
                         <v-pagination
                              ...
                              :color="themeColor == '' ? 'primary' : themeColor"
                         ></v-pagination>
                         <v-card-text class="caption">Total Record(s): {{productcategories.length}}</v-card-text>
                    </v-card>
               </v-container>
          </v-lazy>
                    ...
                    <v-container fluid>
                              <v-form ref="form" v-model="valid" lazy-validation>
                                   <v-row align="center" justify="center" dense>
                                        <v-col cols="12" md="12">
                                             <v-text-field
                                                  v-model="editproductcategory.ProductCategoryDesc"
...
export default {
     data() {
          return {
               ...
               headers: [
                    {text: 'Product Category', value: 'ProductCategoryDesc'},
               ...
               breadCrumbsItems: [
                    ...
                    {text: 'Product Categories', disabled: true, href: '#'}
               ]
          }
     },
     created() {
     ...
     methods: {
          ...
          loadJobAssignments() {
               ...
               this.axios.get(url).then(async res => {
                    this.loading = true
                    this.productcategories = await res.data
                    this.loading = false
               })
          },
          saveRecord() {
               if(this.$refs.form.validate()) {
                    this.swal.fire(this.saveOptions).then(result => {
                         if(result.isConfirmed) {
                              let body = {
                                   procedureName: 'ProcProductCategories',
                                   values: [
                                        this.userInfo.CompanyCode,
                                        this.editproductcategory.ProductCategoryNo,
                                        this.editproductcategory.ProductCategoryDesc,
                                        this.editproductcategory.CreatedDate,
          ...
          newRecord() {
               this.dialog = true
               this.editproductcategory.CompanyCode = this.userInfo.CompanyCode
          },
          editRecord(val) {
               this.editproductcategory = val
               this.dialog = true
               this.editMode = 1
          },
          deleteRecord(val) {
               this.swal.fire({
                    ...
                    if(result.isConfirmed) {
                         let body = {
                              ...
                              values: [
                                   val.CompanyCode,
                                   val.ProductCategoryNo,
                                   val.ProductCategoryDesc,
                                   val.CreatedDate,
                                   this.moment().format('YYYY-MM-DD hh:mm:ss'),
                                   this.userInfo.EmployeeCode,
                                   0
